...
// @ngInject
function ReviewTabController($element, $scope, VirtualThreadList, magnets,
   annotationUI, session) {
  ...
  var magnetList = {};
  magnetList[NaN] = {
    text: "No magnets assigned",
    count: 0,
    annotationThreads: [],
  }

  visibleThreads._rootThread.children.forEach(function (thread) {
    if (thread.annotation.magnets.length === 0) {
      magnetList[NaN].count +=1;
      magnetList[NaN].annotationThreads.push(thread);
    } else {
      thread.annotation.magnets.forEach(function (magnet) {
        if (magnetList[magnet]) {
          magnetList[magnet].count +=1;
          magnetList[magnet].annotationThreads.push(thread);
        } else {
          magnetList[magnet] = {
            text: magnet,
            count: 1,
            annotationThreads: [thread],
          };
        }
      });
    }
  });
  if (magnetList[NaN].count === 0) {
    magnetList.delete(NaN);
  }
...
module.exports = {
  ...
  template: require('../templates/review-tab.html'),
};
